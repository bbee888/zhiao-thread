...
export class DeptService {
  ...
  async updateDept(id: string | number, updateDeptDto: UpdateDeptDto) {
    ...
    if (updateDeptDto.code) {
      ...
      if (existCode && existCode.id !== idNum) {
    ...
    if (updateDeptDto.name) {
      ...
      if (existName && existName.id !== idNum) {
    ...
    await this.deptRepository.update(idNum, updateData);
    return await this.deptRepository.findOne({ where: { id: idNum } });
  }
  ...
  async deleteDept(id: string | number) {
    ...
    const result = await this.deptRepository.update({ id: Number(idStr) }, { isDeleted: 1 });
