...
export class FileService {
  ...
  constructor(
    ...
    // 处理本地存储路径：如果是相对路径，则转换为相对于项目根目录的绝对路径
    if (this.ossType === "local") {
      const storagePath = this.ossConf.local.storagePath;
      // 判断是否为相对路径（不以 / 开头且不是绝对路径格式）
      if (storagePath && !path.isAbsolute(storagePath)) {
        // 将相对路径转换为绝对路径（相对于项目根目录）
        const projectRoot = path.resolve(__dirname, "../../");
        this.ossConf.local.storagePath = path.resolve(projectRoot, storagePath);
      }
    }
