...
export class WxMaAuthService {
  ...
  async phoneLogin(loginCode: string, phoneCode: string): Promise<LoginResultDto> {
    ...
    await this.bindWechatOpenId(String(user.id), openId, session.unionid, session.session_key);

    return this.generateTokenByUserId(String(user.id));
  }
  ...
  async bindMobile(openId: string, mobile: string, smsCode: string): Promise<LoginResultDto> {
    ...
    await this.bindWechatOpenId(String(user.id), openId, null, null);
