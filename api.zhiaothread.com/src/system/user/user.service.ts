...
export class UserService {
  ...
  async getUserPage(
    ...
    depts.forEach((d) => deptMap.set(d.id.toString(), d.name));
  ...
  async findMe(currentUserInfo: CurrentUserInfo): Promise<CurrentUserDto> {
    ...
    const user = await this.userRepository.findOne({
      where: { id: Number(userId.toString()), isDeleted: 0 },
      select: [
        "id",
        "username",
        "nickname",
  ...
  async changeCurrentUserPassword(userId: string, data: PasswordChangeDto): Promise<boolean> {
    ...
    const ok = (result.affected ?? 0) > 0;
    if (ok) {
      await this.invalidateUserSessions(String(user.id));
    }
    return ok;
  }
  ...
  async bindOrChangeMobile(userId: string, data: MobileUpdateDto): Promise<boolean> {
    ...
    const exist = await this.userRepository.findOne({
      where: { mobile, isDeleted: 0, id: Not(Number(userId)) },
      select: ["id"],
    });
    if (exist) {
  ...
  async bindOrChangeEmail(userId: string, data: EmailUpdateDto): Promise<boolean> {
    ...
    const exist = await this.userRepository.findOne({
      where: { email, isDeleted: 0, id: Not(Number(userId)) },
      select: ["id"],
    });
    if (exist) {
  ...
  async importUsersFromBuffer(buffer: Buffer) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      await this.userRoleRepository.save(roleIds.map((rid) => ({ userId: saved.id.toString(), roleId: rid.toString() })));
  ...
  async update(userId: string, updateUserDto: UpdateUserDto) {
    ...
    if (username) {
      ...
      if (existingUser && existingUser.id !== Number(userIdStr)) {
        throw new BusinessException("用户名已存在");
      }
    }

    // 更新用户信息
    const user = await this.userRepository.findOne({ where: { id: Number(userIdStr) } });
    if (!user) {
      throw new BusinessException("用户不存在");
    }
  ...
  async deleteUser(id: string): Promise<boolean> {
    ...
    const user = await this.userRepository.findOne({ where: { id: Number(idStr), isDeleted: 0 } });
    if (!user) {
      return true;
    }
  ...
  async getUserFormData(id: string): Promise<UserFormDto> {
    ...
    const user = await this.userRepository.findOne({
      where: { id: Number(idStr), isDeleted: 0 },
      relations: ["roles"],
    });
    if (!user) {
