...
export class NoticeService {
  ...
  async getNoticeFormData(id: string): Promise<(CreateNoticeDto & { id: string }) | null> {
    ...
    const notice = await this.noticeRepository.findOne({ where: { id: idNum, isDeleted: 0 } });
  ...
  async getNoticeDetail(id: string, userId: string) {
    ...
    const notice = await this.noticeRepository.findOne({ where: { id: Number(idStr), isDeleted: 0 } });
    if (notice) {
      // 标记当前用户的这条通知为已读
  ...
  async publishNotice(id: string, publisherId: string) {
    ...
    if (notice.targetType === 1) {
      ...
      targetUserIds = users.map((u) => u.id.toString());
      ...
      if (targetUserIds.length) {
        ...
        const users = await this.userRepository.find({
          where: { id: In(targetUserIds.map((id) => Number(id))), isDeleted: 0 },
        });
        targetUserIds = users.map((u) => u.id.toString());
      }
    }
    ...
    if (targetUserIds.length) {
      ...
      const records = targetUserIds.map((userId) => {
        ...
        record.noticeId = notice.id.toString();
    ...
    const targetUsernames = (
      await this.userRepository.find({
        where: targetUserIds.map((id) => ({ id: Number(id) })),
      })
    ).map((u) => u.username);
  ...
  async revokeNotice(id: string) {
    const notice = await this.noticeRepository.findOne({
      where: { id: Number(id.toString()), isDeleted: 0 },
    });
    if (!notice) return false;
  ...
  async deleteNotices(ids: string[]) {
    const idNums = (ids || []).map((v) => Number(v));
    await this.noticeRepository.update(idNums, { isDeleted: 1 });
    return true;
  }
