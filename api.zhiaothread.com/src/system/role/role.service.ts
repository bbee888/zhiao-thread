...
export class RoleService {
  ...
  async findRoleIdsByCodes(roleCodes: string[]): Promise<string[]> {
    ...
    return roles.map((r) => r.id.toString());
  }
  ...
  async findRoleIdsByCodesOrNames(codesOrNames: string[]): Promise<string[]> {
    ...
    if (remaining.length) {
      ...
      return [...rolesByCode.map((r) => r.id.toString()), ...rolesByName.map((r) => r.id.toString())];
    }

    return rolesByCode.map((r) => r.id.toString());
  }
  ...
  async saveRole(
    ...
    if (roleId) {
      oldRole = await this.roleRepository.findOne({ where: { id: Number(roleId), isDeleted: 0 } });
      if (!oldRole) {
  ...
  async getRoleForm(id: string) {
    ...
    if (role.dataScope === DataScopeEnum.CUSTOM) {
      const deptIds = await this.getRoleDeptIds(role.id.toString());
      return {
        ...(role as any),
        deptIds,
      };
  ...
  async updateRoleStatus(roleId: string, status: number): Promise<boolean> {
    ...
    const role = await this.roleRepository.findOne({
      where: { id: Number(roleIdStr), isDeleted: 0 },
      select: ["id"],
    });
    ...
    const result = await this.roleRepository.update(Number(roleIdStr), { status: Number(status) });
  ...
  async remove(id: string) {
    return await this.roleRepository.update(Number(id.toString()), { isDeleted: 1 });
  }
  ...
  async deleteRoles(ids: string) {
    ...
    for (const roleId of roleIds) {
      const role = await this.roleRepository.findOne({ where: { id: Number(roleId), isDeleted: 0 } });
      if (!role) {
        continue;
      }
      ...
      await this.roleRepository.update(Number(roleId), { isDeleted: 1 });
   ...
   * 查询角色信息
   */
  async findRolesByIds(roleIds: Array<string | number>): Promise<SysRole[]> {
    const ids = (roleIds || []).map((id) => Number(id)).filter((id) => !Number.isNaN(id));
    return await this.roleRepository.find({
      where: { id: In(ids), isDeleted: 0 },
      select: ["id", "code", "name", "dataScope"],
    });
  }
    ...
    if (customRoleCodes.length > 0) {
      ...
      if (customRoleIds.length > 0) {
        ...
        for (const rd of roleDepts) {
          const roleCode = roleIdToCodeMap.get(Number(rd.roleId));
          if (roleCode) {
   ...
   * 获取角色的数据权限范围
   */
  async getRoleDataScopesByIds(roleIds: string[]): Promise<Map<string, number[]>> {
    const customDeptIdsMap = new Map<string, number[]>();

    if (roleIds.length > 0) {
      // 查询所有角色，包括它们的数据范围
      const roles = await this.roleRepository.find({
        where: { id: In(roleIds.map((id) => Number(id))), isDeleted: 0 },
      });

      // 提取需要查询自定义部门权限的角色
      const customRoles = roles.filter((r) => r.dataScope === DataScopeEnum.CUSTOM);
      const customRoleIds = customRoles.map((r) => r.id);
      const roleIdToCodeMap = new Map(customRoles.map((r) => [r.id, r.code]));
